...
def calculate_score(
    ...
    if context_words:
        ...
        issues.append(
            "Username/common word found: "
            + ", ".join(context_words[:5])
        )
    ...
    if repeated_block:
        score -= 12
        issues.append(
            f"Repeated block found: '{repeated_block}'"
        )
...
def print_report(result):
    ...
    if checks["number_sequence"]:
        print(
            f"  [WARN] Number sequence: "
            f"{checks['number_sequence']}"
        )
    else:
    ...
    if checks["context_words"]:
        print(
            "  [WARN] Common/username words: "
            + ", ".join(checks["context_words"])
        )
    else:
...
def main():
    ...
    try:
        username = input(
            "Username/email (optional, press Enter to skip): "
        ).strip()
    except (EOFError, KeyboardInterrupt):
        print("\nCancelled.")
        return 130
